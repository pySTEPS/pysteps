...
sparse feature-based representation of the input rain rate field. Building on
extrapolation nowcast, the temporal evolution of rainfall fields is modeled in
the Lagrangian coordinates. Using the ARI process is adapted from ANVIL
:cite:`PCLH2020`, and the convolution is adapted from the integro-difference
equation (IDE) methodology proposed in :cite:`FW2005` and :cite:`XWF2005`. The
...
the short-space Fourier transform (SSFT) methodology developed in :cite:`NBSG2017`.
