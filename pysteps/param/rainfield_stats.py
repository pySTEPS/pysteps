# Contains: RainfieldStats (dataclass, from_dict, to_dict), compute_field_parameters, compute_field_stats
"""
...
class RainfieldStats:
    ...
    nonzero_mean_db: Optional[float] = None
    nonzero_stdev_db: Optional[float] = None

    mean_rain: Optional[float] = None
    stdev_rain: Optional[float] = None
    rain_fraction: Optional[float] = None
    ...
    def calc_corl(self):
        ...
        # Make sure that we have defined auto-correlations
        if self.cascade_lag1 is None or self.cascade_lag2 is None:
            self.cascade_corl = None
            return

        # We have defined auto-correlations so set up the correlation length array
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "RainfieldStats":
        ...
        ttime = kwargs["valid_time"]
        if ttime is not None and ttime.tzinfo is None:
            ttime = ttime.replace(tzinfo=datetime.timezone.utc)
            kwargs["valid_time"] = ttime

        ttime = kwargs["base_time"]
        if ttime is not None and ttime.tzinfo is None:
            ttime = ttime.replace(tzinfo=datetime.timezone.utc)
            kwargs["base_time"] = ttime

        # Add cascade_corl explicitly, since it's constructed dynamically
        lag1_list = cascade.get("lag1", [])
        kwargs["cascade_corl"] = [np.nan] * len(lag1_list) if lag1_list else None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "cascade": (
                ...
                if self.cascade_stds is not None
                else None
            ),
            "metadata": {
                "domain": self.domain,
...
def compute_field_parameters(
    db_data: np.ndarray, db_metadata: dict, scalebreak: Optional[float] = None
):
    """
    ...
    field_params = {"power_spectrum": power_spectrum, "cdf": cdf}
    return field_params


def power_spectrum_1D(
    field: np.ndarray, scale_break: Optional[float] = None
    ...
    power_spectrum = power_spectrum[: field.shape[0] // 2, : field.shape[1] // 2]

    n_bins = power_spectrum.shape[0]
    bins = np.logspace(
        np.log10(freq_r.min() + 1 / n_bins), np.log10(freq_r.max()), num=n_bins
    )
    ...
    for i in range(len(bins) - 1):
        ...
        power_1d[i] = np.nanmean(power_spectrum[mask]) if np.any(mask) else np.nan
    ...
    model_params = {
        "beta_1": float(beta_1),
        "beta_2": float(beta_2) if beta_2 is not None else None,
        "c1": float(c1),
        "c2": float(c2) if c2 is not None else None,
        "scale_break": float(scale_break) if scale_break is not None else None,
    }
...
def compute_field_stats(data, geodata):
    nonzero_mask = data >= geodata["threshold"]
    nonzero_mean = np.mean(data[nonzero_mask]) if np.any(nonzero_mask) else np.nan
    nonzero_stdev = np.std(data[nonzero_mask]) if np.any(nonzero_mask) else np.nan
...
def correlation_length(
    lag1: float, lag2: float, dx=10, tol=1e-4, max_lag=1000
) -> float:
    """
...
def power_law_acor(
    config: Dict[str, Any], T_ref: float
