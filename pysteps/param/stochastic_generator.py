...
from models import StochasticRainParameters

def gen_stoch_field(steps_params: StochasticRainParameters, nx: int, ny: int):
    """
    Generate a rain field with normal distribution and a power law power spectrum 
    Args:
        steps_params (StochasticRainParameters): The dataclass with all the steps parameters
    ...
    pixel_size = steps_params.kmperpixel
    scale_break = pixel_size * steps_params.scale_break
    ...
    bins = [res.lowerlimit + ia *
            res.binsize for ia in range(1+res.cumcount.size)]
    ...
    rain_mask = norm_data > steps_params.threshold
    rain_obs = norm_data[rain_mask]
    rain_res = stats.cumfreq(rain_obs, numbins=nbins)
    rain_bins = [rain_res.lowerlimit + ia *
                 rain_res.binsize for ia in range(1+rain_res.cumcount.size)]
    ...
    target_sigma = steps_params.nonzero_stdev_db * 0.80
    normal_values = stats.norm.ppf(
        rain_cdf_clipped, loc=target_mu, scale=target_sigma)

    # Create interpolation function from observed rain values to target normal values
    cdf_transform = interpolate.interp1d(
        bin_centers, normal_values,
...
def normalize_db_field(data, params):
    if params.rain_fraction < 0.025:
        return np.full_like(data, params.zerovalue)
    ...
    norm_data = data + (params.threshold - rain_threshold)

    # Now we need to transform the raining samples to have the desired distribution
    # Get the sample distribution
    rain_mask = norm_data > params.threshold
    rain_obs = norm_data[rain_mask]
    rain_res = stats.cumfreq(rain_obs, numbins=nbins)
    rain_bins = [rain_res.lowerlimit + ia *
                 rain_res.binsize for ia in range(1+rain_res.cumcount.size)]
    ...
    target_sigma = params.nonzero_stdev_db * bias_adj 
    normal_values = stats.norm.ppf(
        rain_cdf_clipped, loc=target_mu, scale=target_sigma)
...
def pl_filter(beta_1: float, nx: int, ny: int, pixel_size: float, beta_2: Optional[float] = None, scale_break: Optional[float] = None,
              ):
    """
