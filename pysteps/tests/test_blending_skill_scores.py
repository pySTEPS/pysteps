...
clim_cor_values_9lev = np.array(
    [0.848, 0.537, 0.237, 0.065, 0.020, 0.0044, 0.0052, 0.0040, 10e-4]
)
...
# TODO: Also test the pysteps.blending.clim.calc_clim_weights functionality in
...
def test_blending_skill_scores(
    ...
    correlations_t0 = spatial_correlation(obs, mod)
    ...
    assert_array_almost_equal(
        correlations_t0,
        expected_cor_t0,
        "Returned spatial correlation is not the same as the expected value",
    )
    ...
    correlations_clim, regr_clim = clim_regr_values(n_cascade_levels, "random_path")
    assert (
    ...
    assert_array_almost_equal(
        correlations_clim,
        clim_cor_values,
        "Not the correct climatological correlations were returned",
    )
    assert_array_almost_equal(
        regr_clim, regr_pars, "Not the correct regression parameters were returned"
    )

    # Then, check the regression of the correlation values
    correlations_nwp_lt = lt_dependent_cor_nwp(lt, correlations_t0, "random_path")
    assert (
    ...
    assert_array_almost_equal(
        correlations_nwp_lt,
        expected_cor_nwp_lt,
        "Correlations of NWP not equal to the expected correlations",
    )

    # Finally, make sure nowcast correlation regresses towards the correct
    # value given some PHI-values.
    correlations_nowcast_lt = lt_dependent_cor_extrapolation(
    ...
    assert_array_almost_equal(
        ...
        expected_cor_nowcast_lt,
        "Correlations of nowcast not equal to the expected correlations",
    )
