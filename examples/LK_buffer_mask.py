...
ref_dbr = precip_dataset[precip_var][0].values.copy()
...
xy, uv = dense_lucaskanade(precip_dataset, dense=False, fd_kwargs=fd_kwargs1)
...
xy, uv = dense_lucaskanade(precip_dataset, dense=False, fd_kwargs=fd_kwargs2)
plt.imshow(ref_dbr, cmap=plt.get_cmap("Greys"))
plt.imshow(mask, cmap=colors.ListedColormap(["black"]), alpha=0.5)
plt.quiver(
    xy[:, 0],
...
dataset_uv1 = dense_lucaskanade(precip_dataset, dense=True, fd_kwargs=fd_kwargs1)
dataset_uv2 = dense_lucaskanade(precip_dataset, dense=True, fd_kwargs=fd_kwargs2)
UV1 = np.stack([dataset_uv1["velocity_x"].values, dataset_uv1["velocity_y"].values])
UV2 = np.stack([dataset_uv2["velocity_x"].values, dataset_uv2["velocity_y"].values])
...
dataset_uv1[precip_var] = dataset_uv1[precip_var].where(
    np.isfinite(dataset_uv1[precip_var]), zerovalue
)
dataset_uv2[precip_var] = dataset_uv2[precip_var].where(
    np.isfinite(dataset_uv2[precip_var]), zerovalue
)

dataset_f1 = extrapolate(dataset_uv1, 12)
dataset_f2 = extrapolate(dataset_uv2, 12)

# Back-transform to rain rate
dataset_f1 = transformation.dB_transform(dataset_f1, threshold=-10.0, inverse=True)
dataset_f2 = transformation.dB_transform(dataset_f2, threshold=-10.0, inverse=True)

R_f1 = dataset_f1[precip_var].values
R_f2 = dataset_f2[precip_var].values
...
precip_dataset_o = io.read_timeseries(fns, importer, **importer_kwargs)
precip_dataset_o = conversion.to_rainrate(precip_dataset_o)
precip_var_o = precip_dataset_o.attrs["precip_var"]
R_o = precip_dataset_o[precip_var_o].values
